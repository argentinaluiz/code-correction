    ...
    conn
      .on('ready', function() {
        ...
        conn.shell(function(err, stream) {
          ...
          stream.write('rm /var/run/docker.pid\n')
          stream.write('service docker restart\n');  
          stream.write('docker ps - a\n');
          stream.end(`timeout 10 docker run --rm -e JAVA_OPTS=-Dhudson.model.ParametersAction.keepUndefinedParameters=true -v /root/workspace:/workspace -v /root/jenkins/plugins:/tmp/jenkins/plugins -v /var/run/docker.sock:/var/run/docker.sock -v /root/build:/tmp/build argentinaluiz/jenkins-runner -p /tmp/jenkins/plugins ${paramsArg} -ns --runWorkspace /tmp/build/test\nexit\n`);
          // stream.end('exit')
          //stream.end('ls -l\nexit\n');
        });
      })
