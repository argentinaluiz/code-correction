...
export class ExerciseResolver {
  ...
  async create(
    ...
    const stream = spawn(
      `ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no root@${ipAddress} service docker restart && docker run --rm -e JAVA_OPTS=-Dhudson.model.ParametersAction.keepUndefinedParameters=true -v /root/workspace/Jenkin:/workspace -v /root/jenkins/plugins:/tmp/jenkins/plugins -v /var/run/docker.sock:/var/run/docker.sock -v /root/build:/tmp/build argentinaluiz/jenkins-runner -p /tmp/jenkins/plugins ${paramsArg} -ns --runWorkspace /tmp/build/test`,
      { shell: true  },
    );
