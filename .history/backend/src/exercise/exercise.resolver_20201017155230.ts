...
export class ExerciseResolver {
  ...
  async create(
    ...
    const stream = spawn(
      `ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no root@${ipAddress} docker run --name jenkins-runner --rm -e JAVA_OPTS="-Dhudson.model.ParametersAction.keepUndefinedParameters=true" -v /root/workspace:/workspace -v /root/jenkins/plugins:/root/jenkins/plugins -v /var/run/docker.sock:/var/run/docker.sock -v /root/build:/root/build argentinaluiz/jenkins-runner -p /root/jenkins/plugins ${paramsArg} -ns --runWorkspace /root/build/test`,
      { shell: true },
    );
