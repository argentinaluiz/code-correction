...
export class ExerciseResolver {
  ...
  async create(
    ...
    console.log(paramsArg, path.replace(/\s/g, '\\ '), `docker run --rm -e JAVA_OPTS="-Dhudson.model.ParametersAction.safeParameters=${this.getParams(join(path, 'Jenkinsfile')).map(param => param.name).join(',')}" -v ${path.replace(
        /\s/g,
        '\\ ',
      )}:/workspace -v ${pathPlugins}:/tmp/jenkins_plugins -v /var/run/docker.sock:/var/run/docker.sock jenkins/jenkinsfile-runner -p /tmp/jenkins_plugins ${paramsArg}`);

    const stream = spawn(
      `docker run --rm -e -v ${path.replace(
        /\s/g,
        '\\ ',
      )}:/workspace -v ${pathPlugins}:/tmp/jenkins_plugins  jenkins/jenkinsfile-runner -p /tmp/jenkins_plugins ${paramsArg}`,
      { shell: true },
    );
