...
export class ExerciseResolver {
  ...
  async create(
    ...
    const stream = spawn(
      `ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no root@${ipAddress} docker run --name jenkins-runner --rm -e JAVA_OPTS="-Dhudson.model.ParametersAction.keepUndefinedParameters=true" -v /tmp/workspace:/workspace -v /tmp/jenkins/plugins:/tmp/jenkins/plugins -v /var/run/docker.sock:/var/run/docker.sock -v /tmp/build:/tmp/build argentinaluiz/jenkins-runner -p /tmp/jenkins_plugins ${paramsArg} -ns --runWorkspace /tmp/build/test`,
      { shell: true },
    );
