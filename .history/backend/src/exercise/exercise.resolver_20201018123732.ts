...
export class ExerciseResolver {
  ...
  async create(
    ...
    console.log(
      'Cloning vm',
      clonevm.stdout.toString(),
      clonevm.output.toString(),
    );
    ...
    console.log(
      `ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no root@${ipAddress} docker run --name jenkins-runner --rm -e JAVA_OPTS="-Dhudson.model.ParametersAction.keepUndefinedParameters=true" -v /tmp/workspace:/workspace -v /tmp/jenkins/plugins:/tmp/jenkins/plugins -v /var/run/docker.sock:/var/run/docker.sock -v /tmp/build:/tmp/build argentinaluiz/jenkins-runner -p /tmp/jenkins/plugins ${paramsArg} -ns --runWorkspace /tmp/build/test`,
    );
    ...
    conn
      .on('ready', function() {
        ...
        conn.shell(function(err, stream) {
          ...
          //stream.write('service docker start\n');  
          //stream.end(`docker run --rm -e JAVA_OPTS=-Dhudson.model.ParametersAction.keepUndefinedParameters=true -v /root/workspace:/workspace -v /root/jenkins/plugins:/tmp/jenkins/plugins -v /var/run/docker.sock:/var/run/docker.sock -v /root/build:/tmp/build argentinaluiz/jenkins-runner -p /tmp/jenkins/plugins ${paramsArg} -ns --runWorkspace /tmp/build/test\nexit\n`);
          stream.write('rm /var/lib/docker/volumes/metadata.db\n');
          stream.write('service docker stop\n');
          stream.write('rm /var/lib/docker/network/files/local-kv.db');
          stream.write('service rm -rf /var/lib/docker\n');
          stream.write('service docker start\n');
          stream.write('sh script.sh\n')
          stream.end('exit')
          //stream.end('ls -l\nexit\n');
        });
      })
