...
export class ExerciseResolver {
  ...
  async create(
    ...
    console.log(`ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no root@${ipAddress} docker run --name jenkins-runner --rm -e JAVA_OPTS="-Dhudson.model.ParametersAction.keepUndefinedParameters=true" -v ${path.replace(
      /\s/g,
      '\\ ',
    )}:/workspace -v ${pathPlugins}:/tmp/jenkins/plugins -v /var/run/docker.sock:/var/run/docker.sock -v /tmp/build:/tmp/build argentinaluiz/jenkins-runner -p /tmp/jenkins/plugins ${paramsArg} -ns --runWorkspace /tmp/build/test`);
    spawnSync(`scp -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -r ${path.replace(/\s/g,'\\ ')}/* root@${ipAddress}:/tmp/workspace`, {shell: true});
    const stream = spawn(
