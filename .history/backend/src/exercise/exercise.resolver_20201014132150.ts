...
export class ExerciseResolver {
  ...
  async create(
    ...
    @Args('params') params: { [key: string]: string }[],
  ) {
    const path = join(
      __dirname,
      '../',
    ...
    const paramsArgs = Object.keys(params).reduce((previousValue, currentValue) => {
        previousValue.push(`${currentValue}=${params[currentValue]}`)
    }, []);
    
    const stream = spawn(
      `docker run --rm -v ${path}:/workspace/Jenkinsfile jenkins/jenkinsfile-runner`,
    );
