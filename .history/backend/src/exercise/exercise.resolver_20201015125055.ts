...
export class ExerciseResolver {
  ...
  async create(
    ...
    const showvminfo = spawnSync(`vboxmanage.exe showvminfo "${cloneName}" | grep MAC | awk '{print $4}' | cut -d',' -f1vboxmanage.exe showvminfo "${cloneName}" | grep "NIC 1:" | awk '{print tolower($4)}' | sed 's/.\{2\}/&:/g' | sed 's/.\{2\}$//'`, {shell: true})
    if(showvminfo.error){
       console.log(showvminfo.error);
       process.exit(1);
    }
    console.log(showvminfo.output.toString());
    const macAddress = showvminfo.stdout.toString().match(/\d+/g).join('').match( /.{1,2}/g ).join( ':' );
