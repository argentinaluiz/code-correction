    ...
    conn
      .on('ready', function() {
        ...
        conn.shell(function(err, stream) {
          ...
          stream.end(`service docker restart && docker run --rm -e JAVA_OPTS=-Dhudson.model.ParametersAction.keepUndefinedParameters=true -v /root/workspace:/workspace -v /root/jenkins/plugins:/tmp/jenkins/plugins -v /var/run/docker.sock:/var/run/docker.sock -v /root/build:/tmp/build argentinaluiz/jenkins-runner -p /tmp/jenkins/plugins ${paramsArg} -ns --runWorkspace /tmp/build/test\nexit\n`);
          // stream.end('exit')
          //stream.end('ls -l\nexit\n');
        });
      })
