  ...
  async create(
    ...
    const stream = spawn(
      `docker run --name jenkins-runner --rm -e JAVA_OPTS="-Dhudson.model.ParametersAction.keepUndefinedParameters=true" -v ${path.replace(
        /\s/g,
        '\\ ',
      )}:/workspace -v ${pathPlugins}:/tmp/jenkins_plugins -v /var/run/docker.sock:/var/run/docker.sock -v /tmp/argentinaluiz/jenkins-runner -p /tmp/jenkins_plugins ${paramsArg} -ns`,
      { shell: true },
    );
