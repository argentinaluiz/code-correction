...
export class ExerciseResolver {
  ...
  async create(
    ...
    if(clonevm.error || clonevm.stderr){
      console.log(clonevm.error, clonevm.stderr.toString());
      process.exit(1);
    }
    console.log('Cloning vm',clonevm.stdout.toString())
    const showvminfo = spawnSync(`vboxmanage showvminfo "${cloneName}" | grep "NIC 1:" | awk '{print tolower($4)}' | sed 's/.\{2\}/&:/g' | sed 's/.\{2\}$//'`, {shell: true})
    if(showvminfo.error || showvminfo.stderr){
       console.log(showvminfo.error, showvminfo.stderr.toString());
       process.exit(1);
    }
