...
export class ExerciseResolver {
  ...
  async create(
    ...
    console.log(paramsArg, path.replace(/\s/g, '\\ '), `docker run --rm -e JAVA_OPTS="-Dhudson.model.ParametersAction.safeParameters=${Object.keys(
        this.getParams(join(path, 'Jenkinsfile')).map(param => param.name),
      ).join(',')}" -v ${path.replace(
        /\s/g,
        '\\ ',
