    ...
    <Dialog open={open} onClose={handleClose}>
      ...
      <DialogContent>
        <form>
          <Grid container>
            <Grid item xs={12}>
              <TextField select label={"Nome"} fullWidth={true} onChange={event => setExerciseSelected(exercises.filter(exercise => exercise.name === event.target.value))}>
                {exercises && exercises.map((exercise: any, key) => (
